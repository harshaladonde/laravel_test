        ...
        <div class="card">
            <div class="card-header">
                ...
                <a class="btn btn-success float-right mb-4" href="javascript:void(0)" id="createNewEmployee">New
                    Registration </a>&nbsp;
                <th><button style="display:none" class="btn btn-danger float-right mb-4 deleteEmployee">Delete all</button>
                </th>&nbsp;
                            ...
                            <tr data-id="{{ $employee->id }}">
                                ...
                                <td>
                                    {{-- <div class="profile-container">
                                        <img src="{{ asset('profiles/' . $employee->profile) }}" alt="Profile Picture" class="profile-image" style="max-width: 50px; max-height:50px;">
                                        
                                    </div> --}}
                                    <div class="profile-container">
                                        <img src="{{ asset('profiles/' . $employee->profile) }}" alt="Profile Picture"
                                            class="profile-image" style="max-width: 50px;max-height: 50px;">
                                        <input type="file" class="profile-input" accept="image/*" style="display: none;">
                                        <input type="hidden" class="profile-filename" value="{{ $employee->profile }}">
                                    </div>
                                </td>
                                <td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="employeeForm" method="POST" action="/employee/store" enctype="multipart/form-data">
                            @csrf
...
<script>
    $(document).ready(function() {
        ...
        //     });
        //     row.find('.profile-container').addClass('editing'); // Add editing class for profile input
        //     row.find('.saveEmployee').show();
        //     $(this).hide();
        // });
        ...
        //     var row = $(this).closest('tr');
        //     var id = row.data('id');
        //     var name = row.find('[data-field="name"] input').val().trim(); // Trim whitespace
        //     var email = row.find('[data-field="email"] input').val().trim();
        //     var phone = row.find('[data-field="phone"] input').val().trim();
        //     var department = row.find('[data-field="department"] input').val().trim();
        //     var hobbies = row.find('[data-field="hobbies"] input').val().trim();

        //     if (!name || !email || !phone || !department || !hobbies) {
        //         alert(
        //         'Please fill in all required fields.'); // Display an alert or handle the validation error
        ...
        //     // Include profile picture handling if needed
        ...
        //         contentType: false,
        //         success: function(response) {
        //             // Update the table row with the saved data
        //             row.find('.editable').each(function() {
        //                 var value = $(this).find('input').val();
        //                 $(this).text(value);
        //             });
        //             $('.editEmployee').show();
        //             row.find('.saveEmployee').hide();
        //             row.find('.profile-container').removeClass('editing');
        //         },
        //         error: function(xhr, status, error) {
        //             console.error(error);
        //             // Handle error messages or display a notification
        //         }
        //     });
        // });
        ...
        $('#employeeForm').submit(function(event) {
            ...
            var profile = $('#profile').prop('files')[0]; // Capture profile picture file
            ...
            $('#hobbiesError').text('');
            $('#profileError').text(''); // Added profile error ID

            // Validate Name, Email, Phone, Department, and Hobbies as before

            // Validate Profile Picture
            if (!profile) {
                $('#profileError').text('Please upload profile picture.');
                return;
            }
            ...
            formData.append('profile', profile); // Append profile picture file to form data
            ...
            $.ajax({
                ...
                success: function(response) {
                    Swal.fire({
                        title: 'Success!',
                        text: response
                        .message, // Assuming the response contains a 'message' field
                        icon: 'success',
